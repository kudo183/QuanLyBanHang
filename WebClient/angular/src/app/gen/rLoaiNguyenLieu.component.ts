...
export class rLoaiNguyenLieuComponent implements AfterViewInit {
  ...
  entities: Array<any>;
  className = 'rLoaiNguyenLieuComponent';
  ...
  onSave(changeSet) {
    this.dataService.save('rLoaiNguyenLieu', changeSet[0], changeSet[1], changeSet[2]).subscribe(p => {
      this.onLoad(undefined);
    });
  }
  ...
  onLoad(event) {
    ...
    this.dataService.get('rLoaiNguyenLieu', qe).subscribe(data => {

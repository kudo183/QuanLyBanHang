...
@Component({
  selector: 'app-rKhoHang',
  templateUrl: './rKhoHang.component.html'
})
export class rKhoHangComponent implements AfterViewInit {
  ...
  onSave(changeSet) {
    this.dataService.save('rKhoHang', changeSet[0], changeSet[1], changeSet[2]).subscribe(p => {
      this.onLoad(undefined);
    });
  }
  ...
  onLoad(event) {
    ...
    this.dataService.get('rKhoHang', qe).subscribe(data => {

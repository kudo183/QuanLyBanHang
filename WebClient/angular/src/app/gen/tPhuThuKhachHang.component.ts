...
@Component({
  selector: 'app-tPhuThuKhachHang',
  templateUrl: './tPhuThuKhachHang.component.html'
})
export class tPhuThuKhachHangComponent implements OnInit, AfterViewInit {
  @ViewChild(HSimpleGridComponent) grid: HSimpleGridComponent;
  @Input() name = 'view_tPhuThuKhachHang';
  @Input() autoLoad = true;
  ...
  entities: Array<any>;
  className = 'tPhuThuKhachHangComponent';
  ...
  onSave(changeSet) {
    this.dataService.save('tPhuThuKhachHang', changeSet[0], changeSet[1], changeSet[2]).subscribe(p => {
  ...
  onLoad(event) {
    ...
    this.dataService.get('tPhuThuKhachHang', qe).subscribe(data => {

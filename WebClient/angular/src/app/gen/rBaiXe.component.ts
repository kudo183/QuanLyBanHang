...
@Component({
  selector: 'app-rBaiXe',
  templateUrl: './rBaiXe.component.html'
})
export class rBaiXeComponent implements OnInit, AfterViewInit {
  @ViewChild(HSimpleGridComponent) grid: HSimpleGridComponent;
  @Input() name = 'view_rBaiXe';
  @Input() autoLoad = true;
  ...
  onSave(changeSet) {
    this.dataService.save('rBaiXe', changeSet[0], changeSet[1], changeSet[2]).subscribe(p => {
  ...
  onLoad(event) {
    ...
    this.dataService.get('rBaiXe', qe).subscribe(data => {

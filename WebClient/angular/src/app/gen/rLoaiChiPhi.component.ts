...
@Component({
  selector: 'app-rLoaiChiPhi',
  templateUrl: './rLoaiChiPhi.component.html'
})
export class rLoaiChiPhiComponent implements OnInit, AfterViewInit {
  @ViewChild(HSimpleGridComponent) grid: HSimpleGridComponent;
  @Input() name = 'view_rLoaiChiPhi';
  @Input() autoLoad = true;
  ...
  entities: Array<any>;
  className = 'rLoaiChiPhiComponent';
  ...
  onSave(changeSet) {
    this.dataService.save('rLoaiChiPhi', changeSet[0], changeSet[1], changeSet[2]).subscribe(p => {
  ...
  onLoad(event) {
    ...
    this.dataService.get('rLoaiChiPhi', qe).subscribe(data => {

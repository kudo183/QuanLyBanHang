...
@Component({
  selector: 'app-rNuoc',
  templateUrl: './rNuoc.component.html'
})
export class rNuocComponent implements OnInit, AfterViewInit {
  @ViewChild(HSimpleGridComponent) grid: HSimpleGridComponent;
  @Input() name = 'view_rNuoc';
  @Input() autoLoad = true;
  ...
  onSave(changeSet) {
    this.dataService.save('rNuoc', changeSet[0], changeSet[1], changeSet[2]).subscribe(p => {
  ...
  onLoad(event) {
    ...
    this.dataService.get('rNuoc', qe).subscribe(data => {

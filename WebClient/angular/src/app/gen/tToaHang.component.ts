...
@Component({
  selector: 'app-tToaHang',
  templateUrl: './tToaHang.component.html'
})
export class tToaHangComponent implements OnInit, AfterViewInit {
  @ViewChild(HSimpleGridComponent) grid: HSimpleGridComponent;
  @Input() name = 'view_tToaHang';
  ...
  onSave(changeSet) {
    this.dataService.save('tToaHang', changeSet[0], changeSet[1], changeSet[2]).subscribe(p => {
      this.onLoad(undefined);
    });
  }
  ...
  onLoad(event) {
    ...
    this.dataService.get('tToaHang', qe).subscribe(data => {

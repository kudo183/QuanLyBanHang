...
@Component({
  selector: 'app-tChiTietChuyenKho',
  templateUrl: './tChiTietChuyenKho.component.html'
})
export class tChiTietChuyenKhoComponent implements OnInit, AfterViewInit {
  @ViewChild(HSimpleGridComponent) grid: HSimpleGridComponent;
  @Input() name = 'view_tChiTietChuyenKho';
  ...
  entities: Array<any>;
  className = 'tChiTietChuyenKhoComponent';
  ...
  onSave(changeSet) {
    this.dataService.save('tChiTietChuyenKho', changeSet[0], changeSet[1], changeSet[2]).subscribe(p => {
      this.onLoad(undefined);
    });
  }
  ...
  onLoad(event) {
    ...
    this.dataService.get('tChiTietChuyenKho', qe).subscribe(data => {

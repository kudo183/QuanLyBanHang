...
@Component({
  selector: 'app-rLoaiHang',
  templateUrl: './rLoaiHang.component.html'
})
export class rLoaiHangComponent implements OnInit, AfterViewInit {
  @ViewChild(HSimpleGridComponent) grid: HSimpleGridComponent;
  @Input() name = 'view_rLoaiHang';
  @Input() autoLoad = true;
  ...
  onSave(changeSet) {
    this.dataService.save('rLoaiHang', changeSet[0], changeSet[1], changeSet[2]).subscribe(p => {
  ...
  onLoad(event) {
    ...
    this.dataService.get('rLoaiHang', qe).subscribe(data => {

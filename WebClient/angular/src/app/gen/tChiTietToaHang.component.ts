...
@Component({
  selector: 'app-tChiTietToaHang',
  templateUrl: './tChiTietToaHang.component.html'
})
export class tChiTietToaHangComponent implements OnInit, AfterViewInit {
  @ViewChild(HSimpleGridComponent) grid: HSimpleGridComponent;
  @Input() name = 'view_tChiTietToaHang';
  ...
  entities: Array<any>;
  className = 'tChiTietToaHangComponent';
  ...
  onSave(changeSet) {
    this.dataService.save('tChiTietToaHang', changeSet[0], changeSet[1], changeSet[2]).subscribe(p => {
      this.onLoad(undefined);
    });
  }
  ...
  onLoad(event) {
    ...
    this.dataService.get('tChiTietToaHang', qe).subscribe(data => {

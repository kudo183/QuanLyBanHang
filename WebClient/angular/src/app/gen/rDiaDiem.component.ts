...
@Component({
  selector: 'app-rDiaDiem',
  templateUrl: './rDiaDiem.component.html'
})
export class rDiaDiemComponent implements OnInit, AfterViewInit {
  @ViewChild(HSimpleGridComponent) grid: HSimpleGridComponent;
  @Input() name = 'view_rDiaDiem';
  ...
  ngAfterViewInit() {
    this.refDataService.gets(['rNuoc']).subscribe(data => {
      this.maNuocSource = data[0].items;
  ...
  onAddingItem(item) {
    item.maNuocSource = this.maNuocSource;
    this.partialMethodService.processItemPartial(this.className, [this, item]);
  }

  onSave(changeSet) {
    this.dataService.save('rDiaDiem', changeSet[0], changeSet[1], changeSet[2]).subscribe(p => {
      this.onLoad(undefined);
    });
  }
  ...
  onLoad(event) {
    ...
    this.dataService.get('rDiaDiem', qe).subscribe(data => {

...
@Component({
  selector: 'app-tMatHang',
  templateUrl: './tMatHang.component.html'
})
export class tMatHangComponent implements OnInit, AfterViewInit {
  @ViewChild(HSimpleGridComponent) grid: HSimpleGridComponent;
  @Input() name = 'view_tMatHang';
  @Input() autoLoad = true;
  ...
  ngAfterViewInit() {
    this.refDataService.gets(['rLoaiHang']).subscribe(data => {
      this.maLoaiSource = data[0].items;
  ...
  onAddingItem(item) {
    item.maLoaiSource = this.maLoaiSource;
    this.partialMethodService.processItemPartial(this.className, [this, item]);
  }

  onSave(changeSet) {
    this.dataService.save('tMatHang', changeSet[0], changeSet[1], changeSet[2]).subscribe(p => {
  ...
  onLoad(event) {
    ...
    this.dataService.get('tMatHang', qe).subscribe(data => {

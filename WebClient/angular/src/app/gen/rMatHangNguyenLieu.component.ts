...
@Component({
  selector: 'app-rMatHangNguyenLieu',
  templateUrl: './rMatHangNguyenLieu.component.html'
})
export class rMatHangNguyenLieuComponent implements OnInit, AfterViewInit {
  @ViewChild(HSimpleGridComponent) grid: HSimpleGridComponent;
  @Input() name = 'view_rMatHangNguyenLieu';
  ...
  entities: Array<any>;
  className = 'rMatHangNguyenLieuComponent';
  ...
  onSave(changeSet) {
    this.dataService.save('rMatHangNguyenLieu', changeSet[0], changeSet[1], changeSet[2]).subscribe(p => {
      this.onLoad(undefined);
    });
  }
  ...
  onLoad(event) {
    ...
    this.dataService.get('rMatHangNguyenLieu', qe).subscribe(data => {

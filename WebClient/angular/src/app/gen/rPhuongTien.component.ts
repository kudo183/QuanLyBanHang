...
@Component({
  selector: 'app-rPhuongTien',
  templateUrl: './rPhuongTien.component.html'
})
export class rPhuongTienComponent implements OnInit, AfterViewInit {
  @ViewChild(HSimpleGridComponent) grid: HSimpleGridComponent;
  @Input() name = 'view_rPhuongTien';
  @Input() autoLoad = true;
  ...
  entities: Array<any>;
  className = 'rPhuongTienComponent';
  ...
  onSave(changeSet) {
    this.dataService.save('rPhuongTien', changeSet[0], changeSet[1], changeSet[2]).subscribe(p => {
  ...
  onLoad(event) {
    ...
    this.dataService.get('rPhuongTien', qe).subscribe(data => {
